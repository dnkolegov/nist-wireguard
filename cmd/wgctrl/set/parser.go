...
const PrivateKeyLenBase64 = 88
const PrivateKeyLen = 66
const PublicKeyLenBase64 = 92
const PublicKeyLen = 67
const PresharedKeyLenBase64 = 44
...
func parsePresharedKeyFile(filePath string) (*wgtypes.Key, error) {
	data, err := ioutil.ReadFile(filePath)
	if err != nil {
		return nil, err
	}

	return parsePresharedKey(string(data))
}
...
func parsePresharedKey(s string) (*wgtypes.Key, error) {
	s = strings.TrimSpace(s)

	if len(s) != PresharedKeyLenBase64 {
		return nil, errors.New("invalid pre-shared key length")
	}
...
func parseCmd(args []string) (*wgtypes.Config, error) {
	...
	for len(args) > 0 {
		if args[0] == "listen-port" && len(args) >= 2 && peer == nil {
			...
			key, err := parsePresharedKeyFile(args[1])
			if err != nil {
				return nil, err
			}
