...
// GenPrivateKey generates a Key suitable for use as a pre-shared secret key from
...
func GeneratePrivateKey() (Key, error) {
	key, _, _, err := elliptic.GenerateKey(Curve, rand.Reader)
	if err != nil {
		return Key{}, err
	}
...
func NewKey(b []byte) (Key, error) {
	keyLen := len(b)
	if keyLen != PrivateKeyLen && keyLen != PublicKeyLen && keyLen != PskLen {
